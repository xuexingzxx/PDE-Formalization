...
\begin{tabular}{@{}llc@{}}
  ...
  \texttt{heatSolution\_solves\_heat} & convolution solves heat eqn ($\partial_t$ proved) & \textcolor{orange}{partial} \\
  \bottomrule
\end{tabular}
\end{center}
...
\textbf{One sorry remains}, \texttt{hspace}: moving the spatial \emph{Laplacian} under the
integral. This is harder --- the Laplacian is the trace of the second Fréchet derivative, so
it needs differentiation under the integral at second order (with second-moment Gaussian
bounds), not covered by the first-order parametric lemma used for \texttt{htime}.
...
$e$ over a backward characteristic cone; that route needs $\int$-energy conservation, the
same differentiation-under-the-integral that blocks the Heat chapter. In one dimension,
however, uniqueness follows from a clean \emph{pointwise} argument via the Riemann
