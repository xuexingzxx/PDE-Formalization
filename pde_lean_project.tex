...
lemma spatialGradient_evansFormula (b : Rn) (g : Rn -> R)
  ...
  -- translation y -> y - t.b has derivative id; chain rule gives fderiv g at the foot
  have hchain : HasFDerivAt (fun y => g (y - t . b)) (fderiv R g (x - t . b)) x := ...
...
lemma timeDerivative_evansFormula (b : Rn) (g : Rn -> R)
  ...
  have hchain : HasDerivAt (fun s => g (x - s . b)) (fderiv R g (x - t . b) (-b)) t :=
    hg.differentiableAt.hasFDerivAt.comp_hasDerivAt t hpsi
  rw [hchain.deriv, map_neg]; congr 1
...
With these in hand, the existence theorem closes by \texttt{simp}:
...
theorem duhamelFormula_solves (b : Rn) (g : Rn -> R)
  ...
  --   + intervalIntegral_comp_comm; spatial LipschitzOnWith via the
  --   mean value inequality with a uniform fderiv bound over a compact tube
...
\texttt{hasDerivAt\_integral\_of\_dominated\_loc\_of\_deriv\_le} separately but no
combined Leibniz rule with a \emph{variable} upper limit. We supply this as
\texttt{Calculus.leibniz\_integral} (a fully proved lemma, no longer a placeholder):
\[
  \frac{d}{dt'}\int_0^{t'} H(t',s)\,ds\Big|_{t'=t}
    = H(t,t) + \int_0^t \partial_1 H(t,s)\,ds .
\]
Its proof avoids the (missing-in-Mathlib) theorem ``continuous partial derivatives
$\Rightarrow$ differentiable'' by the decomposition $F = P + B + C$:
$P(t') = \int_0^{t'} H(t,s)\,ds$ (FTC, gives $H(t,t)$);
$B(t') = \int_0^{t} H(t',s)\,ds - \int_0^t H(t,s)\,ds$ (differentiation under the integral
on the \emph{fixed} interval $[0,t]$, gives $\int_0^t \partial_1 H(t,s)\,ds$); and the
cross-term $C(t') = \int_t^{t'}(H(t',s)-H(t,s))\,ds$, which is $O((t'-t)^2)=o(t'-t)$ and so
has derivative $0$ (lemma \texttt{hasDerivAt\_crossTerm}). The uniform $\partial_1 H$ bound
needed for $B$ and $C$ comes from compactness of a box
...
\texttt{hasFDerivAt\_integral\_of\_dominated\_loc\_of\_lip} needs, for a.e. $s$ in the
integration interval, a Lipschitz bound for $x' \mapsto f(x'-(t-s)b, s)$ valid on the
\emph{whole} ball $B(x,1)$ ---
a single constant, not the center-point value. We obtain it from a uniform bound
$M = \sup \|\text{fderiv}\,f\|$ over the compact ``tube''
$\overline{B}(x,1) \times [0,t]$ (\texttt{IsCompact.exists\_bound\_of\_continuousOn}); then for
each $s$ the mean value inequality
\texttt{Convex.lipschitzOnWith\_of\_nnnorm\_fderiv\_le} on the convex ball gives the
$M$-Lipschitz bound, using
$\|\text{fderiv}\,(\lambda y.\,f(y,s))\,z\| =
\|\text{fderiv}\,f\,(z,s) \circ \text{inl}\| \le \|\text{fderiv}\,f\,(z,s)\|$
(\texttt{norm\_inl\_le\_one}). With the bound now the \emph{constant} $M$, its
\texttt{IntervalIntegrable} hypothesis is just \texttt{intervalIntegrable\_const}.

All other conditions (\texttt{AEStronglyMeasurable} of $F$ and $F'$, the \texttt{toDual} and
\texttt{innerSL} commutations) are fully proved, so \texttt{duhamelFormula\_solves} is now
closed with no \texttt{sorry}.
...
\emph{Location note.} The inner-product CLMs (\texttt{realInnerBiL},
\texttt{realInnerL}, \texttt{realInnerL\_apply}) and the radial-power lemmas
(\texttt{hasFDerivAt\_norm\_rpow\_of\_ne}, \texttt{laplacian\_norm\_rpow\_eq}) shown below
now live in \texttt{Calculus.lean}, shared with the Heat chapter (which defined the same
inner-product CLM independently); only \texttt{laplacian\_const\_mul} and
\texttt{laplacian\_log\_norm\_eq} remain local to \texttt{Laplace.lean}. They are reproduced
here because the harmonicity proof reads most naturally in one place.
...
\begin{lstlisting}[caption={Fundamental solution is harmonic off zero (proved)}]
...
\begin{tabular}{@{}llc@{}}
  ...
  \texttt{integrable\_exp\_neg\_mul\_norm\_sq} & $\int_{\mathbb{R}^n}\!e^{-c|z|^2}<\infty$ & \textcolor{mathgreen}{\checkmark} \\
  \texttt{integrable\_norm\_sq\_mul\_exp\dots} & $\int_{\mathbb{R}^n}\!|z|^2 e^{-c|z|^2}<\infty$ & \textcolor{mathgreen}{\checkmark} \\
  ...
  \texttt{heatSolution\_hasFDerivAt\_grad} & $D_x(\!\int\!\Phi g) = \int g\,D\Phi$ & \textcolor{mathgreen}{\checkmark} \\
...
\textbf{\texttt{htime} (the $t$-derivative under the integral) is now proved.} We apply
Mathlib's \texttt{hasDerivAt\_integral\_of\_dominated\_loc\_of\_deriv\_le} on a compact time
window $(a,b) = (t/2, 2t) \ni t$. The required dominating function comes from
\texttt{heatKernel\_time\_deriv\_bound}, which bounds $|\partial_s\Phi(x-y,s)|$ uniformly over
$s \in [a,b]$ by a fixed Gaussian moment (each $s$-dependent factor --- the power
$(4\pi s)^{-n/2}$, the Gaussian $e^{-|x-y|^2/4s}$, and the polynomial
$|\,|x-y|^2/4s^2 - n/2s|$ --- is bounded at the endpoints). Its integrability over
$\mathbb{R}^n$ reduces, after the translation $y \mapsto x-y$, to the $0$th and $2$nd
Gaussian moments \texttt{integrable\_exp\_neg\_mul\_norm\_sq} and
\texttt{integrable\_norm\_sq\_mul\_exp\_neg\_mul\_norm\_sq} (now in \texttt{Calculus.lean} as
shared tools, since Mathlib has only the base $n$-dim Gaussian and the $1$-D moments). The
kernel's time derivative is fed in
as the \texttt{HasDerivAt} form \texttt{heatKernel\_hasDerivAt\_time}.

\textbf{\texttt{hspace} (the spatial \emph{Laplacian} under the integral) is now also proved,
so the Heat chapter is fully closed (zero \texttt{sorry}).} The Laplacian is the trace
$\Delta F\,x = \sum_i \text{iteratedFDeriv}\,2\,F\,x\,[e_i,e_i]
= \sum_i \text{fderiv}(\text{fderiv}\,F)\,x\,e_i\,e_i$ of the second Fréchet derivative. We move
it under the integral by differentiating \emph{twice}, but always through \emph{single}-CLM
($\mathbb{R}^n \to_L \mathbb{R}$) integrals: the first differentiation
(\texttt{heatSolution\_hasFDerivAt\_grad}) gives the gradient $\int g\,D\Phi$ and, crucially,
that $\text{fderiv}\,F$ is differentiable at $x$; the eval-bridge
$\text{fderiv}(\text{fderiv}\,F)\,x\,e_i\,e_i
= \text{fderiv}\,(z \mapsto \text{fderiv}\,F\,z\,e_i)\,x\,e_i$ (\texttt{fderiv\_fderiv\_apply})
reduces each second partial to a \emph{scalar} directional derivative, which a third
differentiation under the integral (\texttt{heatSolution\_hasFDerivAt\_dirgrad}) lands on the
kernel. Summing over the basis and folding back with
\texttt{laplacian\_eq\_iteratedFDeriv\_stdOrthonormalBasis} gives
$\int g\,\Delta\Phi$, which cancels the time derivative.

This route deliberately avoids the iterated CLM space $\mathbb{R}^n \to_L (\mathbb{R}^n \to_L
\mathbb{R})$: Mathlib lacks the \texttt{ContinuousENorm} instance there (a topology diamond
between the operator-norm topology and the metric topology), so \texttt{Integrable} and
\texttt{ContinuousLinearMap.integral\_apply} are unavailable --- whereas for the single-CLM
dual $\mathbb{R}^n \to_L \mathbb{R}$ they are. Working in single-CLM integrals throughout
sidesteps the gap. The bounds use the $1$st Gaussian moment
\texttt{integrable\_norm\_mul\_exp\_neg\_mul\_norm\_sq} in addition to the $0$th and $2$nd.
...
$e$ over a backward characteristic cone; that route needs $\int$-energy conservation. In one
dimension, however, uniqueness follows from a clean \emph{pointwise} argument via the Riemann
invariants $u_t \pm u_x$, with no integration of the energy required.
...
\texttt{Calculus.lean} also provides a fully proved \emph{combined} Leibniz rule for an
integral with a parameter-dependent integrand \emph{and} a variable upper limit:
\[
  \frac{d}{dt'}\int_0^{t'} H(t',s)\,ds\Big|_{t'=t}
    = H(t,t) + \int_0^t \partial_1 H(t,s)\,ds,
\]
for jointly continuous $H$ and its first partial $\partial_1 H$. Mathlib has the two
ingredients (the fundamental theorem of calculus and differentiation under the integral
over a \emph{fixed} interval) but not their combination, and the naive route is blocked by
the absence of ``continuous partial derivatives $\Rightarrow$ Fréchet differentiable'' on
$\mathbb{R}^2$. We sidestep that with the decomposition $F = P + B + C$ (FTC piece $+$
fixed-interval parametric piece $+$ a cross-term $\int_t^{t'}(H(t',s)-H(t,s))\,ds$ that is
$O((t'-t)^2)$, hence has derivative $0$). The cross-term lemma
\texttt{hasDerivAt\_crossTerm} is the genuinely new piece; the uniform bound on
$\partial_1 H$ it needs comes from compactness. This lemma discharges Step~2
(\texttt{hleibniz}) of \texttt{duhamelFormula\_solves} in the Transport chapter.

\subsection{Shared Analysis Toolkit}

Beyond the spacetime definitions, \texttt{Calculus.lean} collects the
dimension- and chapter-agnostic analysis lemmas that arose while differentiating
convolutions under the integral sign, so the Heat and Laplace chapters can share
them rather than each carrying a private copy. They fall into four groups.

\paragraph{$n$-dimensional Gaussian moments.}
Integrability over $\mathbb{R}^n$ of $\|z\|^k e^{-c\|z\|^2}$ for $k=0,1,2$ and any
$c>0$ (\texttt{integrable\_exp\_neg\_mul\_norm\_sq},
\texttt{integrable\_norm\_mul\_exp\_neg\_mul\_norm\_sq},
\texttt{integrable\_norm\_sq\_mul\_exp\_neg\_mul\_norm\_sq}). Mathlib provides the base
$n$-dim Gaussian and the $1$-D moments only; these are the standard dominating
functions for Gaussian/heat-kernel convolutions.

\paragraph{Real inner product as a bilinear CLM.}
\texttt{realInnerBiL}, \texttt{realInnerL}, \texttt{realInnerL\_apply}, with
\texttt{innerSL\_eq\_realInnerBiL} and \texttt{norm\_realInnerBiL\_apply}
($\|\langle w,\cdot\rangle\| = \|w\|$). Mathlib's \texttt{innerSL R} is
\emph{conjugate}-linear, which obstructs \texttt{fderiv}; this is the same map retyped
as an honest $\mathbb{R}$-bilinear CLM. Previously duplicated verbatim in both Heat and
Laplace.

\paragraph{Radial power calculus.}
\texttt{hasFDerivAt\_norm\_rpow\_of\_ne} ($D(\|x\|^p) = p\|x\|^{p-2}\langle x,\cdot\rangle$)
and \texttt{laplacian\_norm\_rpow\_eq}
($\Delta(\|x\|^p) = p(n+p-2)\|x\|^{p-2}$ for $x\neq 0$, any real $p$). The latter is
\emph{why} $\|x\|^{2-n}$ and $\log\|x\|$ are harmonic, and underlies the Laplace
fundamental solutions.

\paragraph{Continuous-linear-map / second-derivative helpers.}
\texttt{norm\_add\_sq\_le\_two} ($\|a+b\|^2 \le 2\|a\|^2 + 2\|b\|^2$);
\texttt{norm\_smul\_clm\_le} ($\|c \cdot L\| \le \|c\|\,\|L\|$, proved directly so it
applies even to iterated CLM spaces where Mathlib's \texttt{NormSMulClass} instance is
missing); \texttt{norm\_comp\_apply\_le}; the eval-bridge \texttt{fderiv\_fderiv\_apply}
($\text{fderiv}(z\mapsto \text{fderiv}\,h\,z\,v)\,x\,w = \text{fderiv}(\text{fderiv}\,h)\,x\,w\,v$),
which converts a second Fréchet derivative into nested scalar directional derivatives;
and \texttt{laplacian\_integral\_eq}, which moves the whole Laplacian under an integral
sign ($\Delta(\!\int K(\cdot,y))\,x = \int \Delta K(\cdot,y)\,x$) once each diagonal
second directional derivative does, packaging the trace-and-swap plumbing for reuse by
...
\begin{tabular}{@{}llc@{}}
  \toprule
  \textbf{Definition / lemma} & \textbf{Description} & \textbf{Status} \\
  \midrule
