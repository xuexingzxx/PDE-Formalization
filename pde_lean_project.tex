...
theorem duhamelFormula_solves (b : Rn) (g : Rn -> R)
  ...
  sorry -- remaining: hleibniz + technical measurability/integrability conditions
\end{lstlisting}
...
\begin{itemize}
  ...
  \item \textbf{Step 5 technical conditions}: \texttt{AEStronglyMeasurable} of $F'$,
    \texttt{LipschitzOnWith} for the dominated convergence bound, and
    \texttt{IntervalIntegrable} for the bound function and the \texttt{toDual} commutation.
    These follow from $f \in C^1$ but require careful Mathlib API work.
  \item \textbf{Step 6}: continuity of $s \mapsto \nabla_x f(x-(t-s)b, s)$
    needed for \texttt{IntervalIntegrable}; follows from $f \in C^1$.
\end{itemize}
