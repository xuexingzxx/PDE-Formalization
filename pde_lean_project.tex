...
theorem duhamelFormula_solves (b : Rn) (g : Rn -> R)
  ...
  --   + intervalIntegral_comp_comm (sorry: spatial LipschitzOnWith condition)
  -- Step 6 (proved): <Int grad f, b> = Int <grad f, b>
  --   via ContinuousLinearMap.intervalIntegral_comp_comm
  sorry -- remaining: only the spatial-gradient LipschitzOnWith bound condition
\end{lstlisting}
...
\textbf{One sorry remains}, in Step 5: the \texttt{LipschitzOnWith} hypothesis of the
\emph{spatial} \texttt{hasFDerivAt\_integral\_of\_dominated\_loc\_of\_lip}. The placeholder
bound $\|\text{fderiv}\,f\,(x-(t-s)b,s)\|\cdot\|b\|$ only uses the center point; a correct
proof needs a uniform bound $\sup_{x' \in \overline{B}(x,1)}
\|\text{fderiv}\,(\lambda y.\,f(y,s))\,(x'-(t-s)b)\|$ over the compact product
$\overline{B}(x,1) \times [0,t]$ together with
\texttt{Convex.lipschitzOnWith\_of\_nnorm\_hasFDerivWithin\_le} --- the same compactness
technique used in \texttt{leibniz\_integral}.
