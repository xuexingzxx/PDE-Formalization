...
theorem duhamelFormula_solves (b : Rn) (g : Rn -> R)
  ...
  -- Step 1 (proved): chain rule gives
  --   partial_t[f(x-(t-s)b,s)] = -<grad f, b>
  -- Step 2 (sorry): Leibniz rule for d/dt Int_0^t H(t,s) ds
  --   = H(t,t) + Int_0^t partial_t H(t,s) ds
  --   Blocked: Mathlib has FTC and diff-under-integral separately
  --   but no combined Leibniz formula
  -- Step 3 (proved): time derivative of g(x-tb) via chain rule
  -- Step 4 (proved): combine via HasDerivAt.add
  -- Step 5 (sorry): spatial gradient of Duhamel integral
  -- Step 6 (proved): swap <Int grad f, b> and Int<grad f, b>
  --   via ContinuousLinearMap.intervalIntegral_comp_comm
  sorry -- two sorry remaining: hleibniz, hspace
\end{lstlisting}
...
The proof framework is complete: Steps 1, 3, 4, 6 are proved. Two sorry remain:
\begin{itemize}
  ...
  \item \textbf{Step 5} (\texttt{hspace}): spatial gradient of the Duhamel integral,
    requires differentiation under the integral sign for gradients.
\end{itemize}
