...
\begin{tabular}{@{}llc@{}}
  ...
  \texttt{heatSolution\_solves\_heat} & convolution solves heat eqn & \textcolor{orange}{partial} \\
  \bottomrule
\end{tabular}
\end{center}
...
Only the two \emph{differentiation-under-the-integral} identities remain as sorry
(\texttt{htime}: the $t$-derivative lands on the kernel; \texttt{hspace}: the spatial
Laplacian lands on the kernel). These need dominated-convergence bounds for the Gaussian
and its derivatives over all of $\mathbb{R}^n$, plus a growth bound on $g$ (Evans assumes
$g$ bounded continuous; the current statement assumes only continuity). All the kernel
computations they depend on are complete.
