...
\texttt{Calculus.leibniz\_integral} (now a fully proved lemma, no longer a placeholder):
\[
...
\subsection{§2.4.3 Symmetry of Mixed Partials}
...
\partial_x\partial_t u$ is manifest. For a \emph{general} $C^2$ solution this symmetry is the
one extra ingredient (beyond $u_{tt}=u_{xx}$) needed to extend both the energy conservation
law and uniqueness to all solutions. We prove it directly:
...
\texttt{clm\_apply} for evaluating the \texttt{fderiv}-valued map), reducing the claim to
Mathlib's \texttt{second\_derivative\_symmetric}. This is the keystone that would upgrade
\texttt{travelingWaves\_energy\_conservation} and \texttt{travelingWaves\_unique} from the
traveling-wave class to arbitrary $C^2$ solutions; that upgrade is now ``plumbing on top of
the bridge.''
...
existence, regularity, both initial conditions, the energy conservation law, uniqueness for
the Cauchy problem, finite propagation speed, and the symmetry of mixed partials: a complete
formalization of Evans §2.4.1 and §2.4.3 in one space dimension.
