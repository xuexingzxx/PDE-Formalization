...
\begin{tabular}{@{}llc@{}}
  ...
  \texttt{dalembert\_initial\_vel} & $u_t(x,0) = h(x)$ & \textcolor{sorryred}{todo} \\
  \texttt{dalembert\_solves\_wave} & d'Alembert solves wave eqn & \textcolor{sorryred}{todo} \\
  \bottomrule
\end{tabular}
\end{center}

The remaining targets ($u_t(x,0) = h(x)$ via the fundamental theorem of calculus on the
integral term, and that d'Alembert's formula solves the wave equation by decomposing both
the $g$-part and the integral-part into traveling waves) do not depend on any of the
Mathlib blockers, and are natural next steps.
