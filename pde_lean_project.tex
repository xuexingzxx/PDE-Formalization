...
\begin{tabular}{@{}llc@{}}
  ...
  \midrule
  \texttt{fundamentalSolution\_near\_integral\_tendsto\_zero}
    & $\|\Phi\|$ integrable on $B(0,1)$: needs \texttt{integrableOn\_rpow\_norm} & \textcolor{orange}{partial} \\
  \texttt{green\_identity\_annulus}
    & Stokes theorem on spherical domains & \textcolor{orange}{partial} \\
...
The primary blocker for \texttt{green\_identity\_annulus} is Stokes' theorem on spherical
domains, not yet in Mathlib. For \texttt{fundamentalSolution\_near\_integral\_tendsto\_zero},
the missing tool is a Mathlib lemma of the form
\texttt{integrableOn\_rpow\_norm}: $\int_{B(0,r)} \|x\|^s\,dx < \infty$ for $s > -n$.
Both require either polar coordinates in $\mathbb{R}^n$ (only $\mathbb{R}^2$ is in Mathlib)
or dedicated measure-theoretic infrastructure.

The proof framework of \texttt{fundamentalSolution\_near\_integral\_tendsto\_zero} is
otherwise complete: translation invariance (\texttt{integral\_ball\_translate}, proved),
sequential convergence via antitone balls (proved), and transfer to \texttt{nhdsWithin}
(proved). Only the integrability of $\|\Phi\|$ on $B(0,1)$ remains as a sorry.

For \texttt{green\_identity\_annulus}, Step 1 is proved: the algebraic identity
\[
  v\,\Delta u - u\,\Delta v = \sum_i \partial_{e_i}(v\,\partial_{e_i}u - u\,\partial_{e_i}v)
\]
follows from the product rule and \texttt{iteratedFDeriv\_two\_apply}.
Step 2 (Gauss--Green on the annulus) is blocked by Stokes' theorem on spherical domains.
