...
/-!
# Sobolev extension operator (Evans §5.4) — reflection foundations

The extension theorem builds a bounded operator `E : W^{1,p}(Ω) → W^{1,p}(ℝⁿ)` extending functions
off a bounded `C¹` domain, by reflecting across a (locally flattened) boundary.  This file is the
local engine: the **reflection across the coordinate hyperplane `{xᵢ = 0}`** (a linear isometry
equivalence), the one-sided cutoffs and tangential/normal gluing of weak derivatives, the **even
reflection** `evenRefl` and its landing in `W^{1,p}(ℝⁿ)` (`memW1p_evenRefl`, `_restrict`), and the
`Lᵖ` seminorm bounds for the reflection.  The operator assembly (change of variables, boundary
density, the bounded half-space operator, and the general-domain lemmas) is in
`MyProject.Sobolev.ExtensionOperator`.
-/
