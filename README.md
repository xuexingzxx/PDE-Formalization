...
bundled into a bounded linear trace operator `traceCLM`), and the **Sobolev extension operator**
(§5.4, `exists_memW1p_extension`: every `u ∈ W^{1,p}(Ω)` on a bounded `C¹` domain extends to
`W^{1,p}(ℝⁿ)` agreeing a.e. on `Ω` — built by even reflection across a flattened boundary for the flat
half-space local model, then a chart-flatten + partition-of-unity assembly for the general domain).
...
density, which for a general domain needs a Sobolev **extension operator** — and that operator is now
**complete** for arbitrary bounded `C¹` domains (§5.4, `exists_memW1p_extension` in
`Sobolev/ExtensionGeneral.lean`, assembled by chart-flattening + partition-of-unity gluing over the
Chapter-6 boundary-chart infrastructure). So the full trace theorem is no longer blocked: the
remaining work is `C¹`-density up to the boundary (extend to `ℝⁿ`, mollify, restrict) followed by the
bounded-extension-by-density of `traceCLM`.
...
MyProject/
  ...
  Sobolev/                 -- Chapter 5: Sobolev spaces
    ...
    ExtensionGeneral.lean  -- §5.4 general C¹-domain W^{1,p} extension operator (chart flatten + partition-of-unity assembly)
